...
export default function Card({ price, title, img, id, onClick }) {
  // addToCart function needs to be passed
    ...
    <div className={classes.card} id={id}>
      ...
      {!onClick && (
        <button
          ...
          onClick={() => setItemsArr(itemsArr.filter((e) => e.id != id))}
        >
          Remove <Trash2></Trash2>{" "}
        </button>
      )}
    </div>
  );
